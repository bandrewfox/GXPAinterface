...
`r  badger::badge_custom('codecov', paste0(96,'%25'), color = 'green', url = 'https://covr.r-lib.org/')`
<!-- badges: end -->

## Overview

[{GXPAinterface}](https://wfulp.github.io/GXPAinterface/) 
...
Here is a example script to grab two genes from TCGA/GTEX (Xena Toil). This
dataset is open to all without needing to log in and get a token, so you
don't need a token for it. Other datasets may require it, see below for 
...
This template is a parameterized RMarkdown document for pulling data from 
Xpress 2.0, for a given Xpress ID, and uploading to GXPA. 
There are other parameters to specify 
QC entries and additional information you may want to pass to GXPA (i.e. 
addition metadata and series description). The user may chose to load to 
GXPA through this template or save files and load to GXPA manually.

Xpress 2.0 to GXPA template can be used directly and edited as needed. 
For a new report in RStudio select 
**File -> New File -> R Markdown -> From Template -> Xpress to GXPA Workflow**, 
or use the `rmarkdown::draft()` function code.
...
Other examples are provided in the `inst/examples/` folder of the 
[{GXPAinterface}](https://wfulp.github.io/GXPAinterface/) package source.

 - [Preparing Files for GXPA](inst/examples/local_processing.Rmd)
 - [Uploading via GXPAinterface](inst/examples/upload_via_interface.Rmd)

You can copy these to your local folder with the `use_example_rmd()` function:
```{r, eval=FALSE}
GXPAinterface::use_example_rmd(to = "my_examples_folder", overwrite = FALSE)
```
