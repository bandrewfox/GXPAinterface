...
# small helper to rename existing columns to avoid conflicts
column_renamer <- function(input_df, col_name) {
  if (col_name %in% colnames(input_df)) {
    col_indices <- which(colnames(input_df) == col_name)
    new_names <- paste0(col_name, ".", seq_along(col_indices))
    colnames(input_df)[col_indices] <- new_names
    message("Renamed existing '", col_name,
            "' columns to '", paste(new_names, collapse = ", "),
            "' to avoid conflicts.")
  }
  return(input_df)
}
...
#'
#' @param deg_meta A data frame containing score file metadata. Reqd columns:
#'     `score_file_name`, `name`, `description`, `logfc_column`,
#'     `fdr_column`, `group1`, `group2`.
#' @param gene_id_map A data frame mapping original gene IDs to symbols:
#'     `gene_id` (matching original score files) and
#'     `gene_symbol` (final output names).
...
#' @param score_thresholds A list with `logFC` and `FDR` elements for
#'      calculating rank thresholds. Default: `list(logFC = 1, FDR = 0.05)`.
#'
...
process_gxpa_scores <- function(deg_meta,
  ...
  for (i in seq_len(nrow(deg_meta))) {
    ...
    score_data <- utils::read.delim(score_file_path,
                                    stringsAsFactors = FALSE,
                                    check.names = FALSE)

    # if any columns are already named "gene_id", rename them to avoid conflicts
    score_data <- column_renamer(score_data, "gene_id")

    # Assume first column is the gene ID, so rename it to "gene_id"
    colnames(score_data)[1] <- "gene_id"

    # Filter to genes we are keeping
    gene_ids_to_keep <- gene_id_map$gene_id
    frac_found <- sum(gene_ids_to_keep %in% score_data$gene_id) /
      length(gene_ids_to_keep)
    if (frac_found < 0.2) {
      warning(paste0("Only ", round(frac_found * 100, 1),
                     "% of expected IDs found in ",
                     deg_meta$score_file_name[i]))
    }

    score_data <- score_data %>%
      dplyr::filter(.data$gene_id %in% gene_ids_to_keep)

    # if any columns are already named "gene_symbol", "logFC_rank",
    # or "FDR_rank", rename them to avoid conflicts
    for (col_name in c("gene_symbol", "logFC_rank", "FDR_rank")) {
      score_data <- column_renamer(score_data, col_name)
    }
    ...
    score_data <- score_data %>%
      dplyr::mutate(
        logFC_rank = rank(-abs(.data[[logfc_col]]),
                          ties.method = "first", na.last = "keep"),
        FDR_rank   = rank(.data[[fdr_col]],
                          ties.method = "first", na.last = "keep")
      )
    ...
    if (any(is.na(score_data$gene_symbol))) {
      warning("Some gene_ids could not be mapped to ",
              "gene_symbols in ", deg_meta$score_file_name[i])
    ...
    logfc_info_row <- data.frame(
      ...
      rank_threshold =
        min(sum(
                abs(score_data[[logfc_col]]) >= score_thresholds$logFC,
                na.rm = TRUE), 5),
    ...
    fdr_info_row <- data.frame(
      ...
      score_type = "FDR",
      rank_threshold = min(sum(score_data[[fdr_col]] <= score_thresholds$FDR,
                               na.rm = TRUE), 5),
    ...
    utils::write.table(score_data, file = new_file_path,
...
write_mean_sd_scores <- function(tpm_mat, output_dir, prefix = "expr_data") {
  ...
  utils::write.table(expr_means,
                     file = file.path(output_dir, mean_file),
                     sep = "\t", row.names = TRUE, quote = FALSE)
  ...
  utils::write.table(expr_sd,
                     file = file.path(output_dir, sd_file),
                     sep = "\t", row.names = TRUE, quote = FALSE)
